...
> **Upgrading?** As of **0.8.0**, `AddIdevsCorelibServices()` / `RegisterServices()` and the runtime reflection scan (`AddIdevsCorelibLegacyScan()`, the `IdevsCoreLibUseSourceGenerator=false` escape hatch) have been **removed**. Source-generated `AddIdevsServices()` is now the only registration path — call it from `Program.cs`. See the [migration guide](MIGRATION.md#v07x--v080--reflection-removal--misuse-analyzers).
...
From **0.8.0**, `Idevs.Net.CoreLib` bundles four Roslyn misuse analyzers (category `Idevs.DI`) that flag patterns known to cause real-world bugs:
...
```ini
dotnet_diagnostic.IDEVSGEN100.severity = none
```

> Source-generated DI via `AddIdevsServices()` is mandatory as of 0.8.0 — the old `AddIdevsCorelibServices()` reflection path and the `IdevsCoreLibUseSourceGenerator=false` fallback were removed.
